...
public class FileWatcherController {
    ...
    private final FileContentProducer fileContentProducer;

    public FileWatcherController(FileContentProducer fileContentProducer) {
        this.fileContentProducer = fileContentProducer;
    }
            ...
            while ((key = watchService.take()) != null) {
                for (WatchEvent<?> event : key.pollEvents()) {
                    log.debug("Event kind: "
                            ...
                            + "; File affected: "
                            + event.context());
                    fileContentProducer.produce("FILE_CONTENT", event.context().toString());
                }
            }
        } catch (Exception e) {
